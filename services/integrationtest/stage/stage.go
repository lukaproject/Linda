...
func (s *Stage) DeleteBag(bagName string) {
	resp, httpResp := xerr.Must2(
		s.cli.DefaultApi.BagsBagNameDelete(context.Background(), bagName))
	if httpResp.StatusCode != http.StatusOK {
		s.t.Logf(
...
func (s *Stage) WaitForNodeJoinFinished(nodeId, bagName string) (ch chan any) {
	...
	go func() {
		for {
			nodeInfo, httpResp := xerr.Must2(s.cli.DefaultApi.AgentsInfoNodeIdGet(context.Background(), nodeId))
			if httpResp.StatusCode != http.StatusOK {
